...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", env: { "IP_NW" => IP_NW, "IP_START" => IP_START, "NUM_WORKER_NODES" => NUM_WORKER_NODES }, inline: <<-SHELL
      ...
      for i in `seq 1 ${NUM_WORKER_NODES}`; do
        echo "$IP_NW$((IP_START+i)) kworker${i}" >> /etc/hosts
      done
  SHELL
### SSH config for host ###
...
### end ssh config ###
  ...
  config.vm.define "kmaster" do |kmaster|
    ...
    kmaster.vm.provider "virtualbox" do |vb_name_setter|
      vb_name_setter.gui = false
      vb_name_setter.customize ["modifyvm", :id, "--name", "kmaster"]
    end
  ...
  (1..NUM_WORKER_NODES).each do |i|
    ...
    config.vm.define "kworker#{i}" do |node|
      ...
      node.vm.provider "virtualbox" do |vb|
          ...
          vb.customize ["modifyvm", :id, "--name", "kworker#{i}"]
      end
